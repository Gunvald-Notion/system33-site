/* === PROBABILITY HOUSE — TEST STEP (Step 5) — Norwegian ===
   One path, one question = the headline formula of the active path.
   app.no.js calls window.renderTest(path) into #test-host whenever Step 5 opens
...
(function(){
  ...
  function build(path){
    ...
    if(path === 'oppsplitting'){
      ...
      return {
        ...
        drill:{ q:'P(B)', v:s.pb, tol:0.01, sol:'P(A&prime;) = 1 &minus; ' + f(s.pa) + ' = ' + f(s.pna) + '. Summer s\u00e5 de to banene: P(B) = P(B|A)&middot;P(A) + P(B|A&prime;)&middot;P(A&prime;) = ' + f(s.pBgA) + '&middot;' + f(s.pa) + ' + ' + f(s.pBgnA) + '&middot;' + f(s.pna) + ' = ' + f(s.pBgA*s.pa) + ' + ' + f(s.pBgnA*s.pna) + ' = ' + f(s.pb) }
      };
    }
    if(path === 'bayes'){
      var s2 = condScenario();
      return {
        ...
        drill:{ q:'P(A|B)', v:s2.pab/s2.pb, tol:0.02, sol:'Teller P(B|A)&middot;P(A) = ' + f(s2.pBgA) + '&middot;' + f(s2.pa) + ' = ' + f(s2.pab) + ' (det er P(A&cap;B)). S\u00e5 P(A|B) = ' + f(s2.pab) + ' / ' + f(s2.pb) + ' = ' + f(s2.pab/s2.pb) }
      };
    }
    var s3 = baseScenario();
    return {
  ...
  function render(){
    ...
    host.innerHTML =
      '<p>\u00c9n formel, ett svar \u2014 stien du valgte over. Les det oppgitte, l\u00f8s, og trykk s\u00e5 <strong>Enter</strong> eller Sjekk. Bytt sti, og testen f\u00f8lger med.</p>' +
      '<div class="result-box" style="border-left-color:var(--blue)"><div class="result-box-name">' + name + ' \u2014 finn ' + T.drill.q + '</div><div class="result-box-formula" style="margin-bottom:8px;color:var(--accent)">' + T.formula + '</div><div class="result-box-formula" style="color:var(--text)">Oppgitt: &nbsp;' + T.scenario + '</div></div>' +
      '<div style="margin-top:20px"><div class="input-row"><div class="input-group"><label>' + T.drill.q + '</label><input type="number" id="t-ans" step="0.01" placeholder="?"></div><button class="action-btn" id="t-ans-btn">Sjekk</button></div><div id="t-fb"></div></div>' +
      '<div class="btn-row" style="margin-top:22px"><button class="action-btn secondary" id="t-new">\u21bb Nytt scenario</button></div>';
  ...
  function judge(ok, sol){
    ...
    var label = ok ? '&#10003; Riktig' : '&#10007; Ikke helt \u2014 her er gjennomgangen';
  ...
  function checkDrill(){
    ...
    if(isNaN(val)){ document.getElementById('t-fb').innerHTML = '<div class="derivation"><div class="d-line" style="color:var(--muted)">Skriv inn et tall f\u00f8rst.</div></div>'; return; }
